...
async function getDBQa(event) {
    // var res= await db.collection('db_qa').get()
  ...
  const { OPENID } = cloud.getWXContext()
    ...
    await db.collection('db_userinfo').where({
        ...
        data:{
            ...
            purePhoneNumber:res.list[0].data.purePhoneNumber,
            countryCode:res.list[0].data.countryCode,
            phone_mobile_creare_time: db.serverDate(),
            isMember:1,
